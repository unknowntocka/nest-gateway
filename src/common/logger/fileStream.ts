...
import { createWriteStream, rename, stat, WriteStream } from 'fs';
...
const onError = (err) => {
  console.error(
    '%s ERROR %s [chair-looger:buffer_write_stream %s: %s\n%s]',
    new Date().toString(),
    process.pid,
    err.name,
    err.message,
...
const fileExists = async (srcPath: string) => {
...
const fileRename = async (oldPath: string, newPath: string) => {
  return new Promise((resolve) => {
  ...
  _getPlusName() {
    ...
    if (this.options.logRotator.byHour) {
      plusName = `${date.getFullYear()}-${
        date.getMonth() + 1
      }-${date.getDate()}${this.options.logRotator.hourDelimiter}${
        date.getHours
      }`;
    } else {
      plusName = `${date.getFullYear()}-${
